...
import com.spring.vegan.member.dto.ReservDTO;
...
//		session.removeAttribute("user_onDTO");
	...
	public ModelAndView searchProc(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if ( command.equals("email") ) {
			...
			if ( email == null ) {
				mav.addObject("email", "일치하는 이메일이 없습니다.");
				mav.setViewName("/member/searchLoginInfo");
			}else {
...
//		request.getSession().setAttribute("client_onDTO", client_onDTO);
	...
	public ModelAndView reservation(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if ( command == null ) {
			viewName = "/reservation/reserv";
		} else if ( command.equals("payPoint") ) { // 포인트 사용하여 예약금 결제 후 예약 테이블에 저장
			int resto_no = Integer.parseInt(request.getParameter("resto_no"));
			String reserv_sched = request.getParameter("reserv_sched");
			...
			// 나중에는 RequestParam으로 받아오면 됨 지금은 없어서 객체 생성해서 임의의 값 넣음
//			insert into reserv values('user01@mail.com', '100001', 123456, default, to_date('2022/11/12 15:22:03', 'YYYY/MM/DD HH24:MI:SS'), 3, 60000, 'N');
			ReservDTO reservDTO = new ReservDTO(); 
			reservDTO.setU_email(user_onDTO.getU_email());
			reservDTO.setResto_no(resto_no);
//			reservDTO.setReserv_sched(); // java.sql.Date로 값이 넘어오니까 그대로 셋 하면됨
			reservDTO.setReserv_member(reserv_member);
			reservDTO.setReserv_deposit(reserv_deposit);
			reservDTO.setReserv_condition("A"); // 최초 예약금 결제 시 "A"
			
			int result = memberService.payPoint( pointDTO, user_onDTO, reservDTO );
