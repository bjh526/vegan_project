...
public class MemberDAOImpl implements MemberDAO {
	...
	public int chargePoint(Card_payDTO card_payDTO, int u_point) {
		...
		System.out.println("chargePoint result: " + result);
		return result;
	}
	...
	@Override
	public int payPoint(PointDTO pointDTO, User_onDTO user_onDTO, ReservDTO reservDTO) { 
		int result = 0;
		try {
			...
			if ( result == 1 ) {
				...
				if ( result == 1 ) {
					// reserv 테이블에 예약내역 insert
					result = sqlSession.insert("mapper.member.insertReserv", reservDTO);
					if ( result == 1 ) {
						// reserv_history 테이블에 변경내역 insert
						result = sqlSession.insert("mapper.member.insertReserv_history", reservDTO);
					}
				}
			}
		} catch (Exception e) {
