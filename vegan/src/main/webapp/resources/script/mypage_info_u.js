	...
	for ( let i = 0; i < result.length; i++ ) {
     ...
     let frmModify = document.frmModify;
		...
		function duplicate_input() {
			let nick = frmModify.input_nick.value.replaceAll(' ', '');
			if (nick != "") {
				$.ajax({
					type : "POST",
					dataType : "text",
					...
					data : { "input_nick" : frmModify.input_nick.value },
					success : function(data) {
						if (data == 'true') {
							btn_nick.disabled = true;
							frmModify.input_nick.readOnly = true;
		...
		function valid_modify() {
			...
			let pwd = frmModify.input_pwd.value.replaceAll(' ', '');
			let pwd_r = frmModify.input_pwd_r.value.replaceAll(' ', '');
			let addr = frmModify.input_addr.value;
			...
			if (pwd != "" || pwd != null) {
				if (st_txt_pwd_t == 'block' && st_txt_pwd_f == 'none'
					...
					if (name != "" || name != null) {
						if (nick != "" || nick != null) {
							if (tel != "" || tel != null) {
								frmModify.action = "${path }/member/mypage.do?command=modify";
								frmModify.submit();
							}
						}
					}
